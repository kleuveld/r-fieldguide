...
# run diagnosis? if set to false, diagnosis results will be read from CSV files
run_diagnosis <- FALSE


```

# (PART\*) Part 2: phases of your research {-}

# 4. Design {#declaredesign}
...
You can either use existing data, or simulate data; 
or use a combination of the two, which is what I do here.

We use the `fabricatr` package to generate data,
which is part of the DeclareDesign ecosystem.

As a starting-off point, I use [RHoMIS](https://www.rhomis.org/) data,
which is a large dataset on agricultural households in several countries.
Here I use Burundi, 
simply because I used it for a project, and this chapter is based on that work.
...
rhomis_raw <-
  read_csv(
    'https://github.com/kleuveld/r-fieldguide/raw/refs/heads/main/data/RHoMIS_Indicators.csv'
  ) %>%
...
Now, we just start fabricating! 

For now, we pull columns from the raw data,
with each new variable of our new data supplied as an argument to `fabricate()`.
We do some light recoding,
but we don't actually fabricate data yet:
that will come once we need a second round of data collection.
Also note that `fabricatr` comes with
[a lot of functions](https://declaredesign.org/r/fabricatr/reference/index.html) 
...
rhomis <-
  fabricate(
    # first define N (or we could have stated a data frame here)
    N = nrow(rhomis_raw),

    # then list all the variables you want!
    # all these should be vectors of length N

    # village id is just the 18 groups of equal size
    village = rep(1:18, each = N / 18, length.out = N),
    hh = 1:N,
    # food secure and educated are recoded to be binary
    food_secure =recode_values(
      rhomis_raw$HFIAS_status,
      c("FoodSecure", "MildlyFI", "ModeratelyFI") ~ 1,
      "SeverelyFI"  ~ 0,
      NA ~ 0
    ),
    educated = recode_values(
      rhomis_raw$Head_EducationLevel,
      "No_school" ~ 0,
      NA ~0,
      default = 1
    ),
    NFertInput = if_else(
      is.na(rhomis_raw$NFertInput), 0, rhomis_raw$NFertInput
    ),
    LivestockHoldings = rhomis_raw$LivestockHoldings
  ) |> 
  as_tibble() # I like tibbles because the print output is nicer
...
Note that here I start of with my `rhomis` data object, 
and not with `N = ...` as before,
since I already have a data frame with the properties I want.
...
model_simple <- declare_model(
  rhomis %>% select(village, NFertInput),
  effect_size = effect_size_simple,
  potential_outcomes(Y ~ NFertInput + Z * effect_size)
)
...
design_simple <- 
  model_simple + assignment_simple + inquiry_simple + measurement_simple + 
  estimator_simple
...
#options(device = "windows") # On Windows
...
So now we have to add a year to our design. 
This means our model changes: in `fabricatr` terms, we have two levels:
households and years.

The household level, I create as wide version of our two-year dataset:
with  `NFertInput_0` based on what we already have,
and then `NFertInput_1` using `correlate_var()`.

The year level is simply, it just has two values, 0 and 1.

Finally, we create the final data set by crossing these two levels,
and creating a variable `NFertInput` that takes the value of `NFertInput_0` or `NFertInput_1` depending on the year.
...
model_dd <- declare_model(
  ...
  # first add the household level
  # including variables for year 0 and 1
  hh = add_level(
    N = nrow(rhomis),
    village = rhomis$village, 
    NFertInput_0 = rhomis$NFertInput,
    NFertInput_1 = correlate_var(rhomis$NFertInput, rho_dd)
  ),

  # then create the year
  survey_round = add_level(N = 2, nest = FALSE, t = 0:1),

  # and now cross them to create the final data set
  obs = cross_levels(
    by = join_using(hh, t),
    # outcome based on the year
    NFertInput = if_else(
      t == 0,
      NFertInput_0,
      NFertInput_1
    ...
    # then we drop unwanted variables (optional of course)
    NFertInput_0 = NULL,
    NFertInput_1 = NULL,
    obs = NULL,
    survey_round = NULL
...
assignment_dd <-
  declare_assignment(
    treatment_group = cluster_ra(clusters = village, prob = 0.5),
    Z = t * treatment_group
  )
...
design_dd <- 
  model_dd + assignment_dd + inquiry_dd + measurement_dd + estimator_dd
...
This is a bit tricky: before we just added the effect size to everyone. 
But for binary variables,
we only change a 0 to a 1 for a certain share of the treated population.
This means the number of 0s we need to flip to 1s depends on the current 
number of units and the share of 1s in the treatment group.
Let's just work out how to do that first.
The function below flips a certain number of 0s to 1, 
so that the proportion of 1s increases by the effect size.
...
flip_zeroes <- function(x, Z, effect) {
  ...
  # figure out how many zeros to flip
...
This allows us to set an arbitrary sample size, 
even one that's larger than the original rhomis data, 
by sampling with replacement from the original data set.

For this I define a custom step `resampling_matching` that uses a handler function 
`resample_hhs()`, which does the re-sampling.
This function is quite complex, and uses some more advanced `tidyverse` functions,
which are covered in more detail in the data cleaning chapter.
But the idea of this sampling is that:
...
To do this, we run the `resample_data()` function separately for treatment and control
communities,
using the `pmap` approach from the [previous chapter](#programming) 
(for an implementation using a combination of nest and map2 functions,
...
resample_hhs  <- function(df, communities_per_arm, respondents_treat, 
                           respondents_control){

  # first, we prepare the data set:
  # we only observe treated households in treated communities, 
  # but all households in control communities: 
  df <-
    df |>
    filter(vill_treat == 0 | Z == 1)


  # then, create a df of conditions that we can pmap over
  # if we want to vary the number communities between the arms, that would be
  # straightforward to add here as well
  conditions <- tibble(
    treat = c(0,1),
    respondents = c(respondents_control, respondents_treat), 
    communities = communities_per_arm
  )

  # finally we pmap to sample for each condition
  conditions |>
    pmap(
      function(treat, respondents, communities){
        df %>%
          filter(Z == treat) |>
          resample_data(
            N = c(communities, respondents), 
