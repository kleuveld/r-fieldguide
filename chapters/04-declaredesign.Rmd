...
rhomis <-
  ...
  mutate(
    HFIAS_status = factor(
      HFIAS_status, 
      levels = c("FoodSecure", "MildlyFI", "ModeratelyFI", "SeverelyFI")
    ),
  ...
  # there are too many missings in my data;
  # these seem reasonable to assume to be 0 when missing:
  mutate(
    across(
      c(LandOwned, NFertInput, educated),
      ~if_else(is.na(.x), 0, .x)
    )
  ) %>%
...
we only change a 0 to a 1 for a certain share of the population.
