# 3. Programming with R {#programming}
...
There's three main advantages this:
First, it saves you time, as you can reuse code snippets you often need, and you don't have to copy-paste code every time you want to do something similar.
Second, it makes your code more organized and readable, 
as you can give your functions meaningful names, 
and you don't have to scroll through long scripts to find the code you need.
But most importantly, it prevents mistakes:
every time you copy-paste code, you run the risk of failing to make the necessary changes.
...
and making functions more reusable with the `box` package.
...
(why would it?),
it looks for `x1` in the global environment, and it's not there. 
So we have to make sure R waits until it's inside `ggplot()` to look for `x1` in the
data frame.
...
Alternatively, there's something to be said for using quoted variable names in your 
functions.


## Map() {#map}

`map()` works just like a loop, but sometimes faster. It takes two arguments:
...
a number of combinations of independent variables, using the `my_model()` function we made.
I can put the combination of
variables in a list, and then run my model for each element of the list.
The result is a nice list that we can pipe into `modelsummary()` to get a table of results:
...
What If you want to specify more arguments to the function you are using in `map()`?
Let's say I want to calculate the mean of a number of vectors, 
but I want to remove missing values. 
I will have to pass two arguments to `mean()`: the vector, and `na.rm = TRUE`:
...
There's more ways to define this functions, but they work the same:
...
# using purr-style lambda functions, where .x is the current element of the vector
vectors %>% 
...
or pass the column to `map()`, which will iterate over each row. 
...
but my approach here can be more easily expanded to more complex things than `map2()`.
...
best to use the project root folder as search path:
...
You can now store your box_modules in a folder called `box_modules`
and use  `box::use(box_modules/functions)` in any script in your project.
...
project.
You can also merge changes you make to your box modules of one project, 
