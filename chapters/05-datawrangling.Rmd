...
conflict <- 
  raw_data %>% 
  mutate(
    conflict_yn = case_when(
    affect_conflicts == "frequently" ~ 1,
...
years_liv_clean <-
  ...
  mutate(
    years_liv = if_else(
      years_liv > quantile(years_liv, 0.95, na.rm = TRUE),
...
wide_data %>%
  mutate(
    age_1 = if_else(age_1 == -99, NA, age_1),
...
expenditures_raw <- 
  ...
  mutate(
    expenditures_fertilizer = rnorm(n = nrow(.)),
...
wide_data %>%
  ...
  mutate(
    mean_age = rowMeans(across(starts_with("age_")),
                          na.rm=TRUE),
...
you can either use `rowwise()` and`c_across()`, 
or -- if speed is a concern -- `pmap()` 
...
wide_data %>%
  ...
  # compute the max using rowwise and c_across
  rowwise() %>%
  mutate(
    max_age = max(
      c_across(starts_with("age_")),
      na.rm=TRUE
    )
  ) %>%
  ungroup() %>%

  # compute the sd using pmap and an anonymous function
  mutate(
    sd_age = pmap_dbl(
      pick(starts_with("age_")),
      \(...) sd(c(...), na.rm = TRUE)
    )
  ) %>%
  str()
...
Above, I compute max_age using `rowwise()` and `c_across()`, and standard deviation using `pmap()`.
...
`pmap()` is vectorized, so is noticeably faster than `rowwise()`, 
but it's also much less easy to read if you're not deep into the `tidyverse`.
What it basically does, 
`pmap_dbl()` (the pmap variant that outputs a vector of numbers instead of a list) 
takes two arguments: 
- A data frame, the rows of which it will iterate over. 
Here, I use `pick` to select all age variables, but you could also use `select()` or `across()`.  
- A function that takes the columns of the data frame as its arguments. 
Here, I use an anonymous function `\` that uses `(...)` to capture all columns.

 
Another useful way to compute row-wise summary statistics,
is to pivot to long first, and then just use plain-old `summarize()`.
