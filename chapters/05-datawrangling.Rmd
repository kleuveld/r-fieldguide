...
`pmap()` is vectorized, so may be faster, but less easy to read.
Note that `pmap_dbl()` outputs a vector of numbers (doubles) has two arguments:

- The first is the iterabe object, in this case the current data frame 
with only the age variables selected. 
Within dplyr verbs such as mutate, you can use `pick()` to create such a dataframe.
- The second is an anonymous function (here we use the `\` shorthand to declare one),
which takes all the variabes in the data frame as its arguments using `(...)`.
